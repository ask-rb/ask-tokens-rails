...
`0.5.2`. Never skip or jump patch numbers (`0.5.0` → `0.5.3` is wrong), even
...
gemchain guard ask-tokens-rails
gemchain update ask-tokens-rails <new-version> --dry-run
gemchain update ask-tokens-rails <new-version>
```
...
3. Build passes — `bundle exec rake build` (or
   `gem build ask-tokens-rails.gemspec`).
4. Changelog — `Unreleased` entries moved under the new `X.Y.Z` heading with
   the release date.
5. Version — bumped in `lib/ask/tokens/rails/version.rb` by gemchain, not by
   hand.
...
| `lib/ask/tokens/rails/version.rb` at HEAD | `X.Y.Z` |
| Git tag | `vX.Y.Z` |
...
- Several bug fixes ship together: one release, `0.5.0` → `0.5.1` (never
  `0.5.3`).
- New backwards-compatible API: `0.5.1` → `0.6.0`.
